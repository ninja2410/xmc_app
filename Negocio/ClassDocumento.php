...
class Documento
{
  ...
  public function select($id){
    ...
    $conexion->conectar();
    if ($id==-1) {
      $query="SELECT id_documento_digital ID, fecha_creacion FECHA, path, descripcion, nombre CATEGORIA, titulo, id_jugador, id_socio from DOCUMENTO_DIGITAL inner join CATEGORIA_DOCUMENTOS cd on DOCUMENTO_DIGITAL.id_categoria_documentos = cd.id_categoria_documentos WHERE DOCUMENTO_DIGITAL.estado=1;";
      $dt=mysqli_query($conexion->objetoconexion,$query);
    }
    else{
      $query="SELECT id_documento_digital ID, fecha_creacion FECHA, path, titulo, id_jugador,  descripcion, nombre CATEGORIA, DOCUMENTO_DIGITAL.id_categoria_documentos, id_socio from DOCUMENTO_DIGITAL inner join CATEGORIA_DOCUMENTOS cd on DOCUMENTO_DIGITAL.id_categoria_documentos = cd.id_categoria_documentos WHERE id_documento_digital=$id AND DOCUMENTO_DIGITAL.estado=1";
